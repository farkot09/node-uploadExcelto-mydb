...
const leerexcel = (ruta) => {
    ...
    for (let i = 0; i < xlData.length; i++) {
        ...
        const sql = 'INSERT INTO programacion (id_chasis, conductor, cedula, placa, chasis, planilla, fecha) VALUES (?, ?, ?, ?, ?, ?, ?)';
        const values = [0, xlData[i].CONDUCTOR, xlData[i].CEDULA, xlData[i].PLACA, xlData[i].CHASIS, xlData[i].PLANILLA, now.toDateString("yyyy-MM-dd")];
...
const agregarIdChasis = () => {
    const sql = 'SELECT chasis.id, programacion.id_chasis,programacion.chasis from chasis,programacion WHERE programacion.id_chasis = 0 AND chasis.chasis = programacion.chasis';
...
const eliminarDespachados = () => {
    const sql = 'DELETE FROM programacion where chasis IN (SELECT chasis from despachos)';
    connection.query(sql, (error, results) => {
        if (error) {
            console.error(error.message);
            return;
...
const actualizarPlanilla = () => {
    const sql = 'SELECT programacion.planilla,despachos.planilla FROM programacion,despachos ORDER BY despachos.planilla DESC LIMIT 1';
    connection.query(sql, (error, results) => {
        if (error) {
            console.error(error.message);
            return;
    ...
    const sql2 = 'SELECT * FROM programacion where planilla = 0 GROUP BY placa ORDER BY placa DESC';
